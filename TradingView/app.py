...
def api_sources():
    """
    Returns the list of pluggable sources and their supported settings.
    """
    try:
...
def api_symbols():
    """
    Returns the list of supported symbols for a given source.
    """
...
def api_history():
    """
    Returns historical candlestick data for a source, symbol, and timeframe.
    """
    source_name = request.args.get('source')
...
def api_quote():
    """
    Returns a live quote (latest price / active candle) for polling-based sources.
    """
    source_name = request.args.get('source')
    symbol = request.args.get('symbol')
...
def load_env_file():
    """
    Manually parse .env file to load API keys into environment variables.
    """
    try:
...
def call_ai_provider(provider, model, api_key, ollama_url, messages, system_prompt, chart_image=None):
    """
    Dispatches the chat request to the selected AI provider as a stream generator.
    """
    ...
    # 5. OpenRouter
        ...
        headers["X-Title"] = "TradingView Plus"
...
def api_chat():
    """
    API endpoint to handle AI chatbot requests as a stream.
    """
    try:
        ...
        system_prompt = data.get("system_prompt", "You are an expert financial trader and technical analyst. Analyze the chart or answer questions clearly.")
        chart_image = data.get("chart_image")
        
        # Load API keys from environment fallback if empty
        if not api_key:
            if provider == "openai":
        ...
        if provider != "ollama" and not api_key:
            return jsonify({"error": f"{provider.upper()} API 키가 없습니다. 설정 화면 또는 .env 파일에 설정해 주세요."}), 400
